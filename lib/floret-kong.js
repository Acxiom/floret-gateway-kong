...

{
    ...
    KongGateway = class KongGateway {
        ...
        constructor(gatewayName, gatewayURL, gatewayAdminPort, gatewayProxyPort, gatewayType) {
            // console.log('constructor values for gateway:  gatewayName, gatewayURL, gatewayAdminPort, gatewayProxyPort, gatewayType: ', gatewayName, gatewayURL, gatewayAdminPort, gatewayProxyPort, gatewayType)
        ...
        async get(uri, options) {
            ...
            let res = await this.send(mergedOptions).then((res) => res.data).catch((e) => {
        ...
        async getAPIs(apiName) {
            ...
            let apis = await this.send(options).then((res) => res.data).catch((e) => {
                console.log('err: ' + e.message)
            });
            return apis || [];
        }
        ...
        async addRoute({serviceName, protocols = ['http', 'https'], methods = ['GET', 'POST', 'DELETE'], hosts = [], paths = [], strip_path = false, preserve_host = false}) {
            if (!paths.length > 0) {
                return;
            }
        ...
        async addAPI(name, uri, upstreamURL, methods=['GET', 'POST', 'DELETE'], protocol = 'http') {
        ...
        async apiNeedsPatch(options) {
            ...
            if (api) {
                ...
                if (patch.methods && !areEqualArr(api.methods, patch.methods)) {
                    patchBody = patchBody || {};
                ...
                if (patch.uris && !areEqualArr(api.uris, patch.uris)) {
                    patchBody = patchBody || {};
